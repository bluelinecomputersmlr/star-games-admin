...
while($xc = fetch($get))
{
    ...
    } else if(strpos($xc['days'],$day."(CLOSE)") !== false){
        $xc['is_open'] = "0";
        $xc['is_close'] = "0";
        $xc['open'] = "CLOSE";
        $xc['close'] = "CLOSE";
...
$get = query("select * from gametime_delhi order by sort_no");
while($xc = fetch($get))
{
...
usort($data['result'], function ($a, $b) use(&$name){
 //   echo $a[$name].'-'.strtotime(date('m/d/Y').' '.$a[$name]) .'<'. time().'<br>';
  if($a[$name] == "HOLIDAY" || $a[$name] == "CLOSE") { return 1; }  if(strtotime(date('m/d/Y').' '.$a[$name]) < time()) {  return 1;}
...
$get_wallet = fetch(query("select * from users where mobile='$mobile'"));


$data['wallet'] = $get_wallet['wallet']+$get_wallet['winning']+$get_wallet['bonus'];

$data['winning'] = $get_wallet['winning'];
$data['bonus'] = $get_wallet['bonus'];
$data['active'] = $get_wallet['active'];
$data['session'] = $get_wallet['session'];
